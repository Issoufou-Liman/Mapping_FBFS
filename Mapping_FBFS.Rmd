...
spacing: double
subparagraph: yes
abstract: Accurate information on actual areas under given cropping/farming systems
  is an important input for applied agricultural research and policies. Therein, remote
  sensing provides a useful tool for analysing spatial metrics in an explicit manner.
  In this paper, we captured various characteristics of flood-based farming systems
  (FBFS) with remote sensing and applied a probabilistic framework for system analysis.
  FBFS were mapped across the Kisumu County in Kenya by incorporating uncertainties
  was used to describe and interlink various features of FBFS using expert knowledge.
  VI products data were acquired in the forms of normalized difference spectral indices
  (NDSI). These were used as proxies for estimating various metrics of FBFS which
  and were propagated along with the rest of other non-spatial nodes derived from
  expert judgement. We demonstrate that spatial explicit information can be derived
  from remote sensing data as fuzzy linguistic quantifiers which are suitable for
  are useful tools for incorporating uncertainties when mapping complex systems in
  a context of limited and uncertain information. The causal probabilistic reasoning
  embedded in the approach was tested on the Tigray region in Ethiopia; an area with
  totally different settings; and seems to perform incredibly well. Results were transparently
  generated in forms of intermediary prior spatial maps for specific metrics to ultimately
  be aggregated into final posterior maps of plausible areas for flood-based farming
  along with their spatial explicit uncertainties.
  into mapping overlays. A Bayesian network ([BNs](https://en.wikipedia.org/wiki/Bayesian_network))
  3 years (2014-2016) of high-level [MODIS](https://terra.nasa.gov/about/terra-instruments/modis)
  were later used to feed the spatial data nodes of the [BNs](https://en.wikipedia.org/wiki/Bayesian_network)
  representing node states in [BNs](https://en.wikipedia.org/wiki/Bayesian_network).
  When such metrics are available, [BNs](https://en.wikipedia.org/wiki/Bayesian_network)
urlcolor: blue
---
...
```{r fig3,echo=FALSE,fig.width=7.54,fig.height= 5.79,dpi=300, dev='png', fig.ext = 'png',fig.cap="Temporal variability in water and vegetation in Kisumu county, Kenya."}
grid.draw(p)
```
...
afr <- readOGR("data_files/shapefiles/Africa.shp.shp")
...
inst00 <- ggplot()+
  ...
  # geom_point(data = Kisumu_localities[2, ], aes(x = Long, y = Lat), alpha=0.5)+
...
inst0 <- ggplot()+
  ...
  geom_point(data = Kisumu_localities[2, ], aes(x = Long, y = Lat))+
  geom_text(data = Kisumu_localities[2, ], aes(x = Long, y = Lat, label = Place), 
...
inst <- ggplot()+
  ...
  # ggsn::scalebar(location="bottomright", y.min=-0.2, y.max=0, 
  #                 x.min=34, x.max=35, dist=.1, dd2km = TRUE, model='WGS84',
  #                 st.dist=.04, transform=TRUE)+
  ggsn::scalebar(data=fortify(Kisumu), dist = 20, dist_unit = "km",
  ...
  geom_point(data = Kisumu_localities[1, ], aes(x = Long, y = Lat))+
...
inst <- gTree(children = gList(ggplotGrob(inst), gb))
...
veg_coverage <- p
# plot(p)
# labeller = c('Very low\n(lower outliers)', 'Low\n(lower Whisker)', 'Moderately low\n(lower IQR)', 
#              'Moderately high\n(upper IQR)','High\n(upper Whisker)', 'Very high\n(upper outliers)')
...
my_subtitle <- expression(paste('NDVI'))
...
p1 <- rasterVis::gplot(p)+
  ...
  geom_path(data=fortify(Kisumu), aes(x=long, y= lat, group=group), size = 0.1, alpha=0.9, colour='black')

p1 <- gTree(children = gList(ggplotGrob(p1), gb))

p1 <- grid.arrange(inst, p1, layout_matrix =rbind(c(1, 2, 2)))
## Probaility of water ####
...
# #                         gp = gpar(fontfamily='serif',fontsize=16,fontface="bold", col="black"))
# # 
# p$heights = c(7.192/2.0, 7.192/1.95)
# 
...
```{r fig4,echo=FALSE,fig.width=7.54,fig.height= 6,dpi=300,fig.cap="Discrete states of vegetation and water in Kisumu county, Kenya."}
grid.draw(p)
```
...
p0 <- rasterVis::gplot(spatialData, maxpixels=ncell(spatialData))+
  ...
  labs(subtitle='Prior spatial data nodes')+
  theme(axis.title = element_blank())+
  theme(plot.margin = margin(t=0, b=-5))+
...
inst_kisumu <- gTree(children = gList(ggplotGrob(inst_kisumu), gb))

p0 <-grid.arrange(inst_kisumu, p0, layout_matrix =rbind(c(1,1, 2, 2, 2, 2, 2),
                                     c(1,1, 2, 2, 2, 2, 2)))
...
p1 <- rasterVis::gplot(p1, maxpixels=ncell(p1))+
  ...
  theme(axis.text = element_text(size = 8),
        axis.title = element_text(size = 12, face = 'bold'),
        strip.text = element_text(size = 8, face = "bold"),
        legend.text = element_text(family = 'serif', face = 'bold', size = 6),
        plot.title = element_text(size = 14, face = 'bold', color = 'black'),
        plot.subtitle = element_text(size = 12, face = 'italic', color = 'blue'))+
  coord_equal()+
  scale_x_continuous(labels = f(1), breaks = scales::pretty_breaks(n=4), expand = c(0, 0)) +
...
########################
f <- function(lst, id=TRUE){
  lengths <- lapply(lst, length)
  longest <- which.max(lengths)
  # lst[which(lengths==longest)]
  tmp0 <- 1:length(lst)
  names(tmp0) <- names(lst)
  tmp <- 1:length(lst[[longest]])
  sapply(tmp0, function(i){
    out <- 1:length(lst[[i]])
    sapply(tmp, function(j) {
      if (j %in% out){
        if(id){
          out <- j
        } else {
...
states <- nodeStates(network)
states <- states[-length(states)]

states <- f(states, id=F)
states <- cbind(States=1:nrow(states), states)
colnames(states) <- c('State\nvalues', "Flow\naccumulation", "Slope", "Suitable\ntopography",
                      "Soil water\ncontent","Exposure to\nwetness","Suitable soils",
                      "Sensitivity\nto flood", "Vegetation\nsensitivity\nto water\nvariation",
                      "Characteristic \nvegetation", "Power\nof tools", "Water\npresent", 
                      "Temporal\nvariation in\nvegetation\nindex", "Flood at the\nbeginning of\nthe rainy\nseason",  
                      "Flooded \nat some\npoints", "Vegetation\nvariation \ncaused by\nflood")
...
p1 <- arrangeGrob(p0, 
                  ...
                  heights = unit(c(1.81, 1.1, 1.1), c("in", "in", "in"))
...
p1$bottom = grid::textGrob("Long",
                          x=0.5, hjust=0, vjust = 0.5,rot = 0,
                          gp = gpar(fontfamily='serif',fontsize=14,fontface="bold", col="black"))
p1$left = grid::textGrob("Lat",
                        rot = 90,
                        gp = gpar(fontfamily='serif',fontsize=14,fontface="bold", col="black"))
p1$ncol <- 1
...
p1 <- arrangeGrob(p1,
                  ...
                  nrow=2,
                  # widths = unit(c(7.54, 7.54, 7.54), c("in", "in", "in")),
                  heights = unit(c(1.81+1.1+1.1+0.4, 1.1), c("in", "in"))
...
```{r fig5,echo=FALSE,fig.width=7.54,fig.height=1.81+1.1+1.1+0.4+1.1,dpi=300,fig.cap="Prior, posterior, and uncertainity maps of differents spatial data used to feed the BNs for mapping FBFS in Kisumu county, Kenya."}
grid.draw(p1)
```

```{r fig5 Kisumu export,echo=FALSE,include=FALSE}
ggsave(plot=p1, device = 'png', filename = "figures/Kisumu_prior_posterior_uncertainity_plot.png", dpi = 300, width=7.54, height = 1.81+1.1+1.1+0.4+1.1)
```
...
rm(list=setdiff(ls(), c("spatialData", 'spatialDataList', "network", "network_bn_fit", 'my_theme', "gb",
                        'intervals','Kisumu', 'get_coverages', 'Tigray', 'col', 'Ethiopia_L1', 'Kenya_L1', 
                        'afr', 'inst_kisumu')))
gc()
...
p0 <-grid.arrange(inst, p0, layout_matrix =rbind(c(1, 1, 2, 2, 2, 2, 2),
                                                 c(1, 1, 2, 2, 2, 2, 2)))
...
p1 <- arrangeGrob(p0, 
                  ...
                  nrow=3,
                  # widths = unit(c(7.54, 7.54, 7.54), c("in", "in", "in")),
                  heights = unit(c(2.3, 1.35, 1.35), c("in", "in", "in"))
...
p1 <- arrangeGrob(p1,
                  ...
                  nrow=2,
                  # widths = unit(c(7.54, 7.54, 7.54), c("in", "in", "in")),
                  heights = unit(c(5.5, 1.1), c("in", "in"))
...
```{r fig6,echo=FALSE,fig.width=7.54,fig.height=5.5+1.1,dpi=300,fig.cap="Prior, posterior, and uncertainity maps of differents spatial data used to feed the BNs for mapping FBFS in Tigray region, Ethiopia"}
grid.draw(p1)
```

```{r fig6 export Tigray,echo=FALSE,include=FALSE}
ggsave(plot=p1, device = 'png', filename = "figures/Tigray_prior_posterior_uncertainity_plot.png", dpi = 300, width=7.54, height = 5.5+1.1)
```

## Potential of FBFS in Kisumu and Tigray 

```{r FBFS maps Kisumu and Tigray,echo=FALSE,include=FALSE}
source("source_files/inset_maps.R")
text_size <- 2
inst01 <- inst01 + 
  geom_text(data = Kisumu_localities[2, ], aes(x = Long, y = Lat, label = Place), size=text_size,
            position = position_nudge(y = 0.7), hjust=0.5, vjust=0)

inst_kisumu <- inst_kisumu + labs(subtitle='Kisumu referential')+
  geom_text(data = Kisumu_localities[1, ], aes(x = Long, y = Lat, label = "Kisumu County, Kenya"), size=text_size,
            position = position_nudge(y = 0.025), hjust=0.5, vjust=0)
inst_kisumu <- inst_kisumu +
  annotation_custom(
    grob = ggplotGrob(inst01+
                        theme(panel.grid = element_blank(),
                              plot.background = element_rect(fill = 'white'),
                              panel.border = element_blank(),
...
inst02 <- inst02 +
  geom_text(data = Tigray_localities[2, ], aes(x = Long, y = Lat, label = Place),
            size=text_size, position = position_nudge(y = -0.7), hjust=0.5, vjust=1)

inst_tigray <- inst_tigray + labs(subtitle='Tigray referential')+
   geom_text(data = Tigray_localities[1, ], aes(x = Long, y = Lat, label = "Tigray region, Ethiopia"),
            size = text_size, position = position_nudge(y = 0.05), hjust=0.5, vjust=0)
inst_tigray <- inst_tigray +
  annotation_custom(
    grob = ggplotGrob(inst02+
                        theme(panel.grid = element_blank(),
                              plot.background = element_rect(fill = 'white'),
                              panel.border = element_blank(),
                              plot.margin = margin(t = 0, r = 0, b = 0, l = 0)
...
rm(list=setdiff(ls(), c('my_theme', "gb",'Kisumu', 'Tigray', 'col', 'get_coverages', 'Ethiopia_L1', 'Kenya_L1', 
                        'afr', 'inst_kisumu', 'inst_tigray', 'text_size')))
gc()
...
p01 <- rasterVis::gplot(Kisumu_FBFS_pixel_class, maxpixels=ncell(Kisumu_FBFS_pixel_class))+
  ...
  theme(
    axis.title = element_blank(),
    axis.text = element_text(size = 6, margin = margin(r=0)),
    legend.text = element_text(family = 'serif', face = 'bold', size = 4, margin = margin(b=-5)),
    plot.margin = margin(r=0, b=0, l=0),
    plot.title = element_blank(),
    plot.subtitle = element_text(size = 8, face = 'italic', color = 'blue', margin = margin(b=12))
  )+
  coord_equal()+
  scale_x_continuous(labels = f(1), breaks = scales::pretty_breaks(n=5), expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0))+
  labs(subtitle='FBFS Potential')+
  guides(fill=guide_colourbar(barwidth=5/2, barheight = 0.25, label.position="top"))
...
p02 <- rasterVis::gplot(Tigray_FBFS_pixel_class, maxpixels=ncell(Tigray_FBFS_pixel_class))+
  ...
  theme(
    ...
    axis.text = element_text(size = 6, margin = margin(r=0)),
    legend.text = element_text(family = 'serif', face = 'bold', size = 4, margin = margin(b=-5)),
    plot.margin = margin(r=0, b=-20, l=0),
    plot.title = element_blank(),
...
p02 <- p02 + 
  ...
  geom_text(data = Tigray_localities, aes(x = Long, y = Lat, label = Place),size = text_size,
            # position = position_nudge(y = 0.2/3), 
            hjust=1, vjust=1, alpha=0.8)
######################################
...
p0 <- list(ggplotGrob(inst_kisumu), ggplotGrob(p01), ggplotGrob(p11), ggplotGrob(p21))
p0$layout_matrix <- cbind(1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4)
p0 <- do.call(grid.arrange, p0)
p0 <- gTree(children = gList(grid.arrange(p0), gb))
###############################
p1 <- list(ggplotGrob(inst_tigray), ggplotGrob(p02), ggplotGrob(p12), ggplotGrob(p22))
p1$layout_matrix <- cbind(1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4)
p1 <- do.call(grid.arrange, p1)
p1 <- gTree(children = gList(grid.arrange(p1), gb))
p1 <- list(p0, p1)
p1$heights <- c(1.15, 1.67)
p1 <- do.call(grid.arrange, p1)
p1 <- gTree(children = gList(grid.arrange(p1), gb))
```

```{r fig7,echo=FALSE,fig.width=6+0.25,fig.height= 1.15+1.67,dpi=300,fig.cap="Potential areas for FBFS and associated uncertainity in Kisumu County, Kenya and Tigray region, Ethiopia."}
grid.draw(p1)
```

```{r fig7 export,echo=FALSE,include=FALSE}
ggsave(plot=p1, device = 'png', filename = "figures/Kisumu_Tigray_Pixel_is_FBFS_plot.png", dpi = 300, width = 7.54, height = 1.15+1.67)
```
