...
```{tikz fig1, echo=FALSE, fig.cap = "Bayesian Network describing the causal reasoning used for mapping FBFS in Kisumu, Kenya.", fig.ext = 'png', cache=FALSE, eval=TRUE, engine.opts = list(engine='xetex', template = "latex/tikz2pdf.tex")}
...
\nodepart{one}
{\centering\Huge
   Automatic pre-processing of MODIS data \par
}
\nodepart{two}
\begin{itemize}
...
\node[text=darkgray][below left=of datadiscretisation.south](multilayertitle) {\Huge Multi layer procedure};

%\node[fill=white, draw=white, text=red][below right=of datadiscretisation](singlelayertitle) {\Huge Single layer procedure};
\node[text=darkgray][below right=of datadiscretisation.south](singlelayertitle) {\Huge Single layer procedure};
