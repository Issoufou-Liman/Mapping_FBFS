...
# Kenya_L0 <- readRDS('data_files/shapefiles/gadm36_KEN_0_sp.rds')
...
###########################
fort_data <- fortify(Kisumu)
inst <- ggplot()+
  geom_polygon(data=fortify(Kisumu), aes(x=long, y= lat, group=group), size = 0.25, fill='white', colour='black')+
  scalebar(
    # data=fortify(Kisumu), 
           x.min = min(fort_data$long), x.max = max(fort_data$long),
           y.min = min(fort_data$lat), y.max = max(fort_data$lat),
           dist = 15, dist_unit = "km",
           transform = TRUE, model = "WGS84", anchor=c(x=34.830, y=-0.54), 
           height=0.03, st.dist=0.05, st.size=2, border.size =0.25)+
  north(
    # data=fortify(Kisumu), 
    x.min = min(fort_data$long), x.max = max(fort_data$long),
    y.min = min(fort_data$lat), y.max = max(fort_data$lat),
    scale = 0.34, anchor=c(x=34.856, y=-0.35))+
...
# Ethiopia_L0 <- readRDS('data_files/shapefiles/gadm36_ETH_0_sp.rds')
...
inst <- ggplot()+
  ...
  scalebar(
    # data=fortify(Tigray), 
           ...
           transform = TRUE, model = "WGS84", anchor=c(x=37.45, y=12.44), 
           height=0.020, st.dist=0.025, st.size=2, border.size =0.25)+
  north(
    # data=fortify(Tigray),
    x.min = min(fort_data$long), x.max = max(fort_data$long),
    y.min = min(fort_data$lat), y.max = max(fort_data$lat),
        scale = 0.275, anchor=c(x=37.48, y=13.30))+
